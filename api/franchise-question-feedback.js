...
module.exports = async function handler(req, res) {
  ...
  const storeAddress = String(body.storeAddress || '').trim().slice(0, 200);
  ...
  const displayName = storeAddress ? `${learnerName} (${storeAddress})` : learnerName;
  ...
  const notionPayload = {
    ...
    properties: {
      ...
      'Người học': { rich_text: [{ type: 'text', text: { content: displayName } }] },
