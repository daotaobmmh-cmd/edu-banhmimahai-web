...
def create_document():
    ...
    run_sub = p_title_sub.add_run("BÁNH MÌ MÁ HẢI · CỔNG ĐÀO TẠO NHƯỢNG QUYỀN TRỰC TUYẾN")
    ...
    r.bold = True
    r = p_meta.add_run("3 phút 35 giây (Chuẩn 7 phần theo đúng kết cấu UI thực tế trên web)")
    ...
    hdr_titles = ["Phân đoạn & Thời lượng", "Thao tác Quay màn hình Mobile (Tinh gọn)", "Lời thoại Voiceover (Có ngắt nghỉ `/`)"]
    ...
    tips = [
        ("1. Kích hoạt điểm chạm màn hình (Touch Pointer):", "Bật chế độ hiển thị điểm chạm tay trên Android/iOS hoặc chèn hiệu ứng vòng tròn chạm (Circle Pulse) trong CapCut/Premiere khi bấm các nút thao tác."),
        ("2. Tỷ lệ khung hình 9:16 (Màn hình dọc):", "Dùng điện thoại quay toàn bộ thao tác màn hình dọc, tối ưu cho Zalo, Messenger, TikTok, Reels."),
        ("3. Zoom 1.2x – 1.5x vào Bằng khen PNG:", "Tại Phần 6, zoom cận cảnh Họ tên + Địa chỉ xe nhượng quyền + Con dấu Bánh Mì Má Hải trên mẫu chứng nhận."),
        ("4. Nhạc nền (Background Music):", "Nhạc tươi vui, nhẹ nhàng (âm lượng 10% - 15%) để làm nổi bật giọng voiceover.")
    ]
    ...
    print(f"SUCCESS: Script Word saved to Desktop at {desktop_docx}")
